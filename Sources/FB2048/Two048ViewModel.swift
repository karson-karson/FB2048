...
enum Two048Direction {
    case up, down, left, right
}

class Two048ViewModel: ObservableObject {
    ...
    init() {
        if let cachedGrid: [[Int]] = UserDefaults.standard.array(forKey: "grid") as? [[Int]] {
            grid = cachedGrid
        } else {
    ...
    func onSwipe(_ direction: Two048Direction) {
        NNSoundService.shared.playSound(.transition)
        let oldGrid = grid
        switch direction {
        case .up:
            for i in 0..<gridSize {
